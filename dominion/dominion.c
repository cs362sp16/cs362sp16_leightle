	     ...
	     struct gameState *state)
{
	...
	//play card
	if (cardEffect
	    (card, choice1, choice2, choice3, state, handPos,
	     &coin_bonus) < 0) {
		return -1;
	}
...
int playMine(struct gameState *state, int currentPlayer, int choice1,
...
int playRemodel(struct gameState *state, int currentPlayer, int choice1,
		int choice2, int handPos)
{
	int i, j;
	j = state->hand[currentPlayer][choice1];	//store card we will trash
	       ...
	       struct gameState *state, int handPos, int *bonus)
{
	...
	switch (card) {
	case adventurer:
		playAdventurer(currentPlayer, state);	//REFACTOR
		return 0;

	case council_room:
		playCouncil_room(state, currentPlayer, handPos);	//REFACTOR
		return 0;

	case feast:
		playFeast(state, currentPlayer, choice1);	//REFACTOR
		return 0;
	...
	case mine:
		return playMine(state, currentPlayer, choice1, choice2, handPos);	//REFACTOR

	case remodel:
		return playRemodel(state, currentPlayer, choice1, choice2, handPos);	//REFACTOR
		...
		if ((state->discardCount[nextPlayer] +
			...
			if (state->deckCount[nextPlayer] > 0) {
				tributeRevealedCards[0] =
				    state->deck[nextPlayer][state->deckCount
							    [nextPlayer] - 1];
			...
			if (state->deckCount[nextPlayer] == 0) {
				for (i = 0; i < state->discardCount[nextPlayer];
				     i++) {
	...
	case ambassador:
		...
		for (i = 0; i < state->numPlayers; i++) {
			if (i != currentPlayer) {
				gainCard(state->hand[currentPlayer][choice1],
					 state, 0, i);
			}
		}
