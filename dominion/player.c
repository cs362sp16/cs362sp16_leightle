...
int main(int argc, char *argv[])
{
	...
	while (TRUE) {
		...
		if (gameStarted == TRUE && gameOver == TRUE) {
			...
			for (playerNum = 0; playerNum < game->numPlayers;
			     playerNum++) {
		...
		sscanf(line, "%s %d %d %d %d", command, &arg0, &arg1, &arg2,
		       &arg3);
